<template>
  <div>
    <v-app-bar app flat elevation="1" color="white" height="48px">
      <v-tabs fixed-tabs>
        <v-tab
          v-for="item in orderMenuList"
          :key="item.id"
          style="width:20%;min-width:20px;padding:0 "
        >
          {{ item.title }}
        </v-tab>
      </v-tabs>
    </v-app-bar>
    <div class="qm-bscroll-screen-box qm-bscroll-screen-box-py-order">
          ...
          >
            ...
            <v-card-text class="pb-1">
              <v-row align="center" class="mx-0 ">
                <div class="caption">订单编号：{{ item.ID }}</div>
                <v-spacer></v-spacer>
                <div class="green--text caption">{{ item.state }}</div>
              </v-row>
            </v-card-text>
            ...
            <v-list two-line>
              <template v-for="(ite, index) in item.goodList">
                <v-list-item>
                  <v-list-item-avatar tile size="54px" class="my-2 mr-2 ">
                    <img :src="ite.avatar" />
                  </v-list-item-avatar>
                  <v-list-item-content>
                    <v-list-item-title
                    ...
                    <v-list-item-subtitle
                      class="caption text-line2-ellipsis grey--text"
                      v-html="ite.subtitle"
                    ></v-list-item-subtitle>
                  </v-list-item-content>
                  <div class="ml-1">
                    ...
                    <v-list-item-subtitle class="text-right overline"
                      >x{{ ite.num }}</v-list-item-subtitle
                    >
                  </div>
                </v-list-item>
                <v-divider
            ...
            <v-card-text class="pt-0">
              <v-chip-group
                ...
                column
              >
                <v-btn small outlined class="ml-auto mr-2" color="grey"
                  >查询账单结果</v-btn
                >
                <v-btn outlined small color="primary">再来一单</v-btn>
              </v-chip-group>
            </v-card-text>
          </v-card>
        </div>
...
export default {
  data() {
    return {
      orderMenuList: [
        { id: 0, title: '全部', link: '/order/list' },
        { id: 1, title: '待付款', link: '/order/list' },
        { id: 2, title: '待收货', link: '/order/list' },
        { id: 3, title: '已完成', link: '/order/list' },
        { id: 4, title: '已退货', link: '/order/list' }
      ],
      loading: false,
      selection: 1,
      orderList: [
  ...
  mounted() {
    const bscroll = document.querySelector('.qm-bscroll-screen')
...
.qm-bscroll-screen-box-py-order {
  padding: 48px 0 56px;
}
</style>
