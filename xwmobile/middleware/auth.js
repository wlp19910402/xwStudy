...
export default async function ({ route, store, redirect }) {
  ...
  if (!store.state.user.user_name) {
    ...
    let res = await axios.get(`${ process.env.baseUrl }/admin/token/${ localStorage.token_value }`);
    if (res.status === 200) {
      if (res.data.err > 0) {
        ...
        let userLogin = { user_name: res.data.user_name, nick_name: res.data.nick_name }
